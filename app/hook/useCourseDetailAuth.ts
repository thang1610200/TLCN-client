...
const useCourseDetailAuth = (slug?: string, email?: string, token?: string) => {
    const { data, error, isLoading, mutate} = useSwr<Course, AxiosError>(token ? [`${BACKEND_URL}/course/detail-course-auth?course_slug=${slug}&email=${email}`,token]: null, fetcher, {
